...
def reset_level():
    enemy_group.empty() # delete all the instances of the sprites
    bullet_group.empty()
    grenade_group.empty()
...
class Soldier(pygame.sprite.Sprite):
    def __init__(self, char_type, x, y, scale, speed, ammo, grenades):
        ...
        self.vision = pygame.Rect(0, 0, 150, 20) # for the enemy --> vision rectangle
        self.move_counter = 0
        self.idling = False
        ...
        for animation in animation_types:
            ...
            for i in range(num_of_frames):
                img = pygame.image.load(f'img/{self.char_type}/{animation}/{i}.png').convert_alpha() # Loads the image that you want # idle image
    ...
    def move(self,moving_left,moving_right):
        ...
        for tile in world.obstacle_list:
            ...
            if tile[1].colliderect(self.rect.x + dx, self.rect.y, self.width, self.height):
                dx = 0
                # if the ai has hit the a wall then make it turn around
        ...
        if pygame.sprite.spritecollide(self, water_group, False): # collisioin  between sprite and a group
            self.health = 0
        ...
        #update scroll based on pkayer position 
        if self.char_type == 'player':
    ...
    def shoot(self): # created a separate class cause player and enemy both can shoot
        if self.shoot_cooldown ==0 and self.ammo > 0:
            self.shoot_cooldown = 20
            bullet =  Bullet(self.rect.centerx + (0.75 * self.rect.size[0] * self.direction), self.rect.centery, self.direction) # size[0] gives x direction size
    ...
    def ai(self):
        if self.alive and player.alive:
            if self.idling == False and random.randint(1, 200) == 1: # any random integer is fine
            ...
            if self.vision.colliderect(player.rect):
                ...
                if self.idling == False:
                    ...
                    self.vision.center = (self.rect.centerx + 75 * self.direction, self.rect.centery) # offset by half the width of the rectangle
                    # pygame.draw.rect(screen, RED, self.vision) # the vision rectangle
...
class Bullet(pygame.sprite.Sprite):
    def __init__(self, x, y, direction):
        pygame.sprite.Sprite.__init__(self)
        self.speed = 10
         # speed for each bullet is same i.e 10
    ...
    def update(self):
        ...
        if pygame.sprite.spritecollide(player, bullet_group, False): # checks collision between a sprite and a group
            if player.alive:
                player.health -= 5
                self.kill()
        for enemy in enemy_group:
            if pygame.sprite.spritecollide(enemy, bullet_group, False): # checks collision between a sprite and a group
                if enemy.alive:
                    enemy.health -= 25 
                    self.kill()
...
class Grenade(pygame.sprite.Sprite):
    def __init__(self, x, y, direction):
        pygame.sprite.Sprite.__init__(self)
        self.timer = 100
        self.vel_y = -11 # velocity of grenade thrown up
        self.speed = 7 # horizontal speed for each grenade is same i.e 7
...
while run:
    ...
    if start_game == False:
        ...
        # show grenades
        draw_text('GRENADES: ',font, WHITE, 10, 69)
        ...
        if player.alive:
            ...
            # throw grenades
                ...
                grenade = Grenade(player.rect.centerx +(0.5* player.rect.size[0]*player.direction),\
                            player.rect.top, player.direction) # new technqie backslash and enter --> helps to split line over two
